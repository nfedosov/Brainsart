...
void BiquadFilter::BilinearTransformation(int filter_type, double A, double B, double C, double& out_a, double& out_b, double& out_c)
{
    ...
    if (filter_type == FILTER_TYPE_NOTCH)
    {
        out_b = -2.0 * (4.0 * A + C);
    }
}
...
bool ButterworthFilter::CreateNotch(double centerFreq_rads, double Q, int N)//order
{
    ...
    double wc_sqr = centerFreq_rads*centerFreq_rads;
    double wc = centerFreq_rads;

    double b0 = 4+wc_sqr;
    double b1 = wc_sqr*2-8;
    double b2 = b0;
    double a0 = 4+wc_sqr+wc/Q;
    double a1 =wc_sqr*2-8;
    double a2 =4+wc_sqr-wc/Q;
