...
class CFIRFit(QWidget):
    ...
    def init_ui(self):
        ...
        self.layout.addWidget(self.perform_button)
        ...
        self.layout.addWidget(self.enter_component_num)
    ...
    def fit_kalman_filter(self):
        ...
        self.filtered_eeg = sn.lfilter(self.b_low,self.a_low,self.filtered_eeg)
        self.filtered_eeg = sn.lfilter(self.b_high,self.a_high,self.filtered_eeg)
        self.filtered_eeg = sn.lfilter(self.b_50,self.a_50,self.filtered_eeg)
        self.filtered_eeg = sn.lfilter(self.b_100,self.a_100,self.filtered_eeg)
        self.filtered_eeg = sn.lfilter(self.b_150,self.a_150,self.filtered_eeg)
        self.filtered_eeg = sn.lfilter(self.b_200,self.a_200,self.filtered_eeg)
        '''
        ...
        #self.plot_corr_lat(self.cmplx_gt_eeg, self.cmplx_filtered_eeg)
    ...
    def plot_raw_psd(self):
        ...
        ic_cond2.plot_psd(ax = ax, picks = [0],color = 'blue', spatial_colors = False,fmin  = 2.0,fmax = 30)
            ...
            #psd_alpha_open = ics_cond1.compute_psd(fmin = 9.0, fmax = 13.0,picks = [i,]).get_data(return_freqs = True)
           
            #psd_alpha_close = ics_cond2.compute_psd(fmin = 9.0, fmax = 13.0,picks = [i,]).get_data(return_freqs = True)
...
class SpatialWindow(QWidget):
    ...
    def init_ui(self):
        ...
        #self.qTable.setColumnWidth(0, 100)
        #self.qTable.setColumnWidth(1, 200)
        #self.qTable.setColumnWidth(2, 500)

        self.set_time_mode(self.time_mode)
        self.time_mode = False
        #####################
        #self.qTable.current_row = None
        ...
        #self.qTable.verticalHeader().setStretchLastSection(True)
        
       
        #qGroupButton.setChecked(true);
    ...
    def set_time_mode(self, flag = False):
        ...
        if flag == True:
            ...
            for ind in range(self.qTable.rowCount()):
                #self.qTable.removeCellWidget(ind,2)
                #self.qTable.setCellWidget(ind,2,self.plot_items[ind])
            ...
            if self.cond_N == 2:    
                #ics = self.ica.get_sources(self.eeg_data)
                #self.ica.plot_sources(self.eeg_data)  #CHECK THERE are
                ...
                #ics
                #ics = self.filters@ self.eeg_data[:,:][0]
                raw_cond1 = mne.Epochs(self.eeg_data,events[0], tmin = 0, tmax = self.duration1-5.0, event_id=events[1][self.exp_info['blocks']['Cond1']['message']], baseline = None)
                raw_cond2 = mne.Epochs(self.eeg_data,events[0], tmin = 5.0, tmax = self.duration2, event_id=events[1][self.exp_info['blocks']['Cond2']['message']],baseline = None)
                
                
                #if self.ica:
                    #self.ica.apply(ics_cond1.load_data())
                    #ics_cond2 = self.ica.apply(ics_cond2.load_data())
                
                    
                #np_cond1 = self.filters@ ics_cond1.get_data()[0]
                #np_cond2 = self.filters@ ics_cond2.get_data()[0]
                #ics_cond1.data_ = np_cond1
                #ics_cond2.data_ = np_cond2
                ...
                #plt.figure()
                #fig, axs = plt.subplots(3, 3) #make several windows if many components
                ...
                for i in range(self.N_rows):
                    #self.qTable.removeCellWidget(i,2)
                    
                    #self.qTable.setCellWidget(i,2,self.psd_plot_items[i])
                    
                    
                    
                    #psds, freqs = mne.time_frequency.psd_welch(ics_cond1, picks = [i,],fmin = 2.0,fmax = 30,n_per_seg = int(self.srate*2),n_fft= int(self.srate*2))
                    ...
                    #psds, freqs = mne.time_frequency.psd_welch(ics_cond2, picks = [i,],fmin = 2.0,fmax = 30,n_per_seg = int(self.srate*2),n_fft= int(self.srate*2))
                    ...
                    #ics_cond1.plot_psd(ax = axs[i//3, i%3], picks = [i,], color = 'red', spatial_colors = False,fmin = 2.0,fmax = 30)
                    #ics_cond2.plot_psd(ax = axs[i//3, i%3], picks = [i,], color = 'blue', spatial_colors = False,fmin  = 2.0,fmax = 30)
    ...
    def plot_spatial_filter(self):
        ...
        ics_cond1 = mne.Epochs(ics,events[0], tmin = 0, tmax = self.duration, event_id = events[1]['Open'], baseline = None)
        ics_cond2 = mne.Epochs(ics,events[0], tmin = 0, tmax = self.duration, event_id = events[1]['Close'],baseline = None)
...
class ICADialog(QDialog):
    def __init__(self, parent):
        ...
        self.setWindowTitle('ICA Results')
        self.setGeometry(200, 200, 400, 300)

        self.layout = QVBoxLayout()

        self.result_label = QLabel('ICA results will be displayed here.')
